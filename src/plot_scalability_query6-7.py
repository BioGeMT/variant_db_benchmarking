...
# ----------------------------------------------------------------------
# 2. Figure 5: Aggregation workloads (Q6, Q7)
# ----------------------------------------------------------------------
queries_fig5 = ["Q6", "Q7"]
n_queries = len(queries_fig5)
...
# ----------------------------------------------------------------------
for i, q in enumerate(queries_fig5):
    ...
    for sys in systems:
        ...
        ax_cold_lin.plot(fractions, y_vals, 
                        ...
                        label=sys)  # Always add label for each system
...
# ----------------------------------------------------------------------
# 5. Create legend manually with all systems (including vcf-miner)
# ----------------------------------------------------------------------
legend_elements = [Line2D([], [], color=color_map[sys], linewidth=line_width[sys],
                          alpha=alpha, linestyle=line_style[sys],
                          marker=marker_style[sys], markersize=marker_size[sys],
                          label=sys)
                   for sys in systems]
...
# ----------------------------------------------------------------------
plt.tight_layout()
plt.savefig("plots/paper_figures/scalability_q6-7.png", dpi=300, bbox_inches="tight")
plt.savefig("plots/paper_figures/scalability_q6-7.pdf", bbox_inches="tight")
plt.show()

print("Figure 5 generated successfully!")
print("Layout: 2 rows (Q6, Q7) × 4 columns (Cold-Linear, Cold-Log, Warm-Linear, Warm-Log)")
print("Legend includes ALL 5 systems (vcf-miner appears even with missing Q6 data)")
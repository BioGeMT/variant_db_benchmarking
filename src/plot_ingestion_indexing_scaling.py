...
for i, solution in enumerate(solutions):
    ...
    ax1.plot(solution_data['sample_percentage'], ingestion_hours, marker=markers[i % len(markers)], linestyle=linestyles[i % len(linestyles)], markersize=ms, linewidth=lw, label=solution)
...
plt.tight_layout()
plt.savefig('plots/ingestion_scaling.png', dpi=300, bbox_inches='tight')
plt.close()
...
for i, solution in enumerate(solutions):
    ...
    lw, ms = (2, 6) if solution == 'vcf2db' else (1, 4)
    ax2.plot(solution_data['sample_percentage'], indexing_hours, marker=markers[i % len(markers)], linestyle=linestyles[i % len(linestyles)], markersize=ms, linewidth=lw, label=solution)
...
plt.tight_layout()
plt.savefig('plots/indexing_scaling.png', dpi=300, bbox_inches='tight')
plt.close()
